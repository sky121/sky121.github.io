...
- **localStorage keys:** `eats-gmaps-key` (the API key — never committed), `eats-prefs` (last swipe preferences), `eats-visited` (the rating log). `sh-theme` (shared site light/dark).
...
- **Decision screen** (on like): "Tonight: <name>" → Open in Maps (Directions), Call (if phone), "I ate here → Rate" (opens the rating sheet), Keep looking. **End-of-deck** screen → Widen preferences / Search farther (live) / Start over.
...
- A **Liked shortlist** (swipe-right to collect several, then compare) as an alternative to the immediate decision screen.
- **Undo last swipe**, and a "seen already" memory so re-opening doesn't repeat passes.
